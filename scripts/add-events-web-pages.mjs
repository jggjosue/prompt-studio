#!/usr/bin/env node
/**
 * Adds 15 events web pages: HTML demos, JSON entries, and PNG previews.
 */
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { eventsPages } from './events-web-pages-data.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.join(__dirname, '..');
const webpagesDir = path.join(root, 'public/webpages');
const jsonPath = path.join(webpagesDir, 'web-pages.json');

function buildDescription(page) {
  const { brand, theme, category, events, chips, dark } = page;
  const mode = dark ? 'dark mode' : 'light mode';
  const eventList = events
    .map(e => `- **${e.title}** (${e.date}, ${e.venue}): ${e.price}`)
    .join('\n');

  return `Create a single-page ${mode} landing for "${brand}", a ${category} events platform / organizer site. Pure HTML + CSS in one index.html file (no frameworks).

**Palette:** background ${theme.bg}, text ${theme.text}, muted ${theme.muted}, accent ${theme.accent}.
**Typography:** Google Fonts — "${theme.sans}"${theme.serif ? ` + "${theme.serif}" for headlines` : ''}.

**Sections (in order):**
1. Sticky nav: logo "${brand}", links (${page.sections.join(', ')}), "Get tickets" or "Register" CTA in accent.
2. Hero: date/location pill "${page.tagline}", large headline with one italic keyword, subcopy, primary CTA + secondary link.
3. Category chips: ${chips.join(', ')}.
4. Featured events grid (3 cards):
${eventList}
Each card: calendar-style date block, title, venue, price, "Buy tickets" button.
5. Stats band: 3 metrics (e.g. "200+ events/year", "50k attendees", "12 cities").
6. Footer: newsletter signup, social links, copyright.

Event ticketing / festival aesthetic: energetic, date-forward, clear CTAs. Optional countdown timer in hero (CSS only). Responsive (768px). Returns index.html only.`;
}

function fontParam(name) {
  const map = {
    'Playfair Display': 'Playfair+Display:ital,wght@0,400;0,700;1,400',
    'Cormorant Garamond': 'Cormorant+Garamond:ital,wght@0,400;0,600;1,400',
    'Space Grotesk': 'Space+Grotesk:wght@400;500;600;700',
    'IBM Plex Mono': 'IBM+Plex+Mono:wght@400;500',
    Inter: 'Inter:wght@400;500;600;700',
  };
  return map[name] || 'Inter:wght@400;500;600;700';
}

function buildHtml(page) {
  const t = page.theme;
  const heroHtml = page.hero.replace(
    /<em>(.*?)<\/em>/g,
    '<em style="font-style:italic;color:var(--accent)">$1</em>'
  );
  const headlineFont = t.serif || t.sans;
  const fonts = t.serif
    ? `${fontParam(t.serif)}&family=${fontParam(t.sans)}`
    : fontParam(t.sans);
  const fontUrl = `https://fonts.googleapis.com/css2?family=${fonts}&display=swap`;
  const btnStyle = page.dark
    ? 'background:var(--accent);color:var(--bg);'
    : 'background:var(--accent);color:#fff;';

  const chips = page.chips.map(c => `<span class="chip">${c}</span>`).join('');
  const navLinks = page.sections.map(s => `<a href="#">${s}</a>`).join('');
  const cards = page.events
    .map(
      e => `<article class="event-card">
      <div class="date-block"><span>${e.date}</span></div>
      <h3>${e.title}</h3>
      <p class="venue">${e.venue}</p>
      <p class="price">${e.price}</p>
      <a href="#" class="ticket-btn">Buy tickets</a>
    </article>`
    )
    .join('\n    ');

  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>${page.brand} — ${page.category}</title>
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
  <link href="${fontUrl}" rel="stylesheet" />
  <style>
    :root {
      --bg: ${t.bg};
      --text: ${t.text};
      --muted: ${t.muted};
      --accent: ${t.accent};
    }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body {
      font-family: "${t.sans}", system-ui, sans-serif;
      background: var(--bg);
      color: var(--text);
      line-height: 1.6;
      font-size: 15px;
    }
    .headline { font-family: "${headlineFont}", Georgia, serif; }
    a { color: inherit; text-decoration: none; }
    .container { max-width: 1100px; margin: 0 auto; padding: 0 1.5rem; }
    nav {
      display: flex;
      justify-content: space-between;
      align-items: center;
      padding: 1rem 1.5rem;
      max-width: 1100px;
      margin: 0 auto;
      position: sticky;
      top: 0;
      z-index: 10;
      background: color-mix(in srgb, var(--bg) 90%, transparent);
      backdrop-filter: blur(10px);
      border-bottom: 1px solid color-mix(in srgb, var(--text) 8%, transparent);
    }
    nav .brand { font-weight: 700; font-size: 1.15rem; }
    nav .links { display: flex; gap: 1.5rem; align-items: center; font-size: 0.9rem; flex-wrap: wrap; }
    nav .links a:hover { color: var(--accent); }
    .btn-tickets {
      padding: 0.5rem 1rem;
      border-radius: 8px;
      font-weight: 600;
      font-size: 0.875rem;
      ${btnStyle}
      border: none;
    }
    .hero {
      padding: 3.5rem 1.5rem 2rem;
      max-width: 820px;
      margin: 0 auto;
      text-align: center;
    }
    .pill {
      display: inline-block;
      font-size: 0.75rem;
      letter-spacing: 0.06em;
      text-transform: uppercase;
      padding: 0.4rem 0.85rem;
      border-radius: 999px;
      border: 1px solid color-mix(in srgb, var(--accent) 50%, transparent);
      color: var(--accent);
      margin-bottom: 1.25rem;
    }
    .hero h1 {
      font-family: "${headlineFont}", Georgia, serif;
      font-size: clamp(2rem, 5vw, 3.1rem);
      font-weight: 400;
      line-height: 1.12;
      margin-bottom: 0.75rem;
    }
    .hero .sub { color: var(--muted); max-width: 44ch; margin: 0 auto 1.5rem; }
    .hero-actions { display: flex; gap: 0.75rem; justify-content: center; flex-wrap: wrap; }
    .btn-primary {
      padding: 0.75rem 1.35rem;
      border-radius: 8px;
      font-weight: 600;
      ${btnStyle}
      border: none;
    }
    .btn-ghost {
      padding: 0.75rem 1.35rem;
      border-radius: 8px;
      border: 1px solid color-mix(in srgb, var(--text) 25%, transparent);
    }
    .chips {
      display: flex;
      flex-wrap: wrap;
      gap: 0.5rem;
      justify-content: center;
      padding: 1.25rem 1.5rem 0;
    }
    .chip {
      font-size: 0.8rem;
      padding: 0.35rem 0.8rem;
      border-radius: 6px;
      background: color-mix(in srgb, var(--accent) 15%, transparent);
    }
    .events-grid {
      display: grid;
      grid-template-columns: repeat(3, 1fr);
      gap: 1.25rem;
      padding: 2.5rem 0 3rem;
    }
    .event-card {
      padding: 1.25rem;
      border-radius: 12px;
      border: 1px solid color-mix(in srgb, var(--text) 12%, transparent);
      background: color-mix(in srgb, var(--bg) 70%, var(--accent) 6%);
      transition: transform 0.2s, border-color 0.2s;
    }
    .event-card:hover {
      transform: translateY(-4px);
      border-color: var(--accent);
    }
    .date-block {
      display: inline-block;
      font-size: 0.7rem;
      font-weight: 700;
      letter-spacing: 0.1em;
      color: var(--accent);
      margin-bottom: 0.75rem;
      padding: 0.25rem 0.5rem;
      border: 1px solid color-mix(in srgb, var(--accent) 40%, transparent);
      border-radius: 4px;
    }
    .event-card h3 { font-size: 1.1rem; margin-bottom: 0.35rem; }
    .venue { font-size: 0.85rem; color: var(--muted); margin-bottom: 0.5rem; }
    .price { font-weight: 600; font-size: 0.9rem; margin-bottom: 1rem; }
    .ticket-btn {
      font-size: 0.85rem;
      font-weight: 600;
      color: var(--accent);
    }
    .stats {
      display: grid;
      grid-template-columns: repeat(3, 1fr);
      text-align: center;
      padding: 2.5rem 0;
      border-top: 1px solid color-mix(in srgb, var(--text) 10%, transparent);
      border-bottom: 1px solid color-mix(in srgb, var(--text) 10%, transparent);
    }
    .stats strong { display: block; font-size: 1.75rem; color: var(--accent); margin-bottom: 0.25rem; }
    .stats span { font-size: 0.85rem; color: var(--muted); }
    footer {
      padding: 2.5rem 1.5rem;
      text-align: center;
      font-size: 0.85rem;
      color: var(--muted);
    }
    footer input {
      margin-top: 0.75rem;
      padding: 0.5rem 1rem;
      border-radius: 6px;
      border: 1px solid color-mix(in srgb, var(--text) 20%, transparent);
      background: transparent;
      color: var(--text);
      width: min(280px, 100%);
    }
    @media (max-width: 768px) {
      nav .links a:not(.btn-tickets) { display: none; }
      .events-grid, .stats { grid-template-columns: 1fr; }
    }
  </style>
</head>
<body>
  <nav>
    <span class="brand">${page.brand}</span>
    <div class="links">
      ${navLinks}
      <a href="#" class="btn-tickets">Get tickets</a>
    </div>
  </nav>
  <section class="hero">
    <span class="pill">${page.tagline}</span>
    <h1 class="headline">${heroHtml}</h1>
    <p class="sub">Discover upcoming ${page.category.toLowerCase()} experiences near you.</p>
    <div class="hero-actions">
      <a href="#" class="btn-primary">Browse events</a>
      <a href="#" class="btn-ghost">View calendar</a>
    </div>
  </section>
  <div class="chips">${chips}</div>
  <section class="events-grid container">
    ${cards}
  </section>
  <section class="stats container">
    <div><strong>200+</strong><span>Events per year</span></div>
    <div><strong>50k</strong><span>Attendees</span></div>
    <div><strong>12</strong><span>Cities</span></div>
  </section>
  <footer class="container">
    <p>Get alerts for new ${page.category.toLowerCase()} near you</p>
    <input type="email" placeholder="your@email.com" aria-label="Email" />
    <p style="margin-top:1.25rem">© ${new Date().getFullYear()} ${page.brand}. Events landing demo.</p>
  </footer>
</body>
</html>`;
}

function sanitizeHtml(html) {
  return html
    .replace(/<div class="hero-actions">/g, '<div class="hero-actions">')
    .replace(/<div><strong>/g, '<div><strong>')
    .replace(/<\/motion>/g, '</motion>')
    .replace(/<\/motion>/g, '</div>');
}

async function generatePreviews(pages) {
  let playwright;
  try {
    playwright = await import('playwright');
  } catch {
    console.warn('playwright not installed; skipping PNG previews.');
    return;
  }

  let browser;
  try {
    browser = await playwright.chromium.launch();
  } catch (err) {
    console.warn('Could not launch Chromium:', err.message);
    return;
  }

  const context = await browser.newContext({ viewport: { width: 1536, height: 1024 } });

  for (const page of pages) {
    const htmlPath = path.join(webpagesDir, page.slug, 'index.html');
    const pngPath = path.join(
      webpagesDir,
      `placeholder-web-${page.id}-${page.slug}.png`
    );
    const tab = await context.newPage();
    await tab.goto(`file://${htmlPath}`, { waitUntil: 'networkidle' });
    await tab.waitForTimeout(500);
    await tab.screenshot({ path: pngPath, type: 'png' });
    await tab.close();
    console.log(`preview: ${path.basename(pngPath)}`);
  }

  await browser.close();
}

async function main() {
  const data = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
  const existingIds = new Set((data.webPages || []).map(p => p.id));
  const added = [];

  for (const page of eventsPages) {
    if (existingIds.has(page.id)) {
      console.log(`skip id ${page.id} (already exists)`);
      continue;
    }

    const slugDir = path.join(webpagesDir, page.slug);
    fs.mkdirSync(slugDir, { recursive: true });
    fs.writeFileSync(
      path.join(slugDir, 'index.html'),
      sanitizeHtml(buildHtml(page))
    );
    added.push(page);
    console.log(`html: ${page.slug}/index.html`);
  }

  if (added.length === 0) {
    console.log('No new entries to add.');
    return;
  }

  await generatePreviews(added);

  const newEntries = added.map(page => ({
    id: page.id,
    title: page.title,
    description: buildDescription(page),
    imageUrl: `/webpages/placeholder-web-${page.id}-${page.slug}.png`,
    imageHint: page.imageHint,
    demoUrl: `/webpages/${page.slug}/index.html`,
    stack: page.stack,
    tags: page.tags,
    membership: 'Premium',
  }));

  data.webPages.push(...newEntries);
  fs.writeFileSync(jsonPath, JSON.stringify(data, null, 2) + '\n');
  console.log(`Added ${newEntries.length} entries to web-pages.json`);
}

main().catch(err => {
  console.error(err);
  process.exit(1);
});
